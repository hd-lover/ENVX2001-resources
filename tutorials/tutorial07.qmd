...
- $\beta_0$ is the intercept where the regression line crosses the y-axis (the expected timber Volume when patch Diameter is 0)
...
$$
Abundance = \beta_0 + \beta_1\cdot \ Diameter + \epsilon_{i}
$$
...
- $y_{i}$ is the response variable (timber volume) for the $i$th observation when the predictor X = $x_i$ (Height or Diameter)
...
- $\beta_2$ is the slope of the regression line for the first predictor, which represents the change in Volume for a one-unit increase in tree Height
- $\epsilon_{i}$ is the random error term for the ith observation and is assumed to be normally distributed with mean 0 and constant variance 

OR if written in terms of the variables in our dataset, the model would be:

$$
\ timber \ Volume = \beta_0 + \beta_1\cdot \ Height + \beta_2\cdot \ Diameter + \epsilon_{i}
$$
...
$H_0: \beta_1 = 0$ , The artial regression slope is equal to zero, i.e. there is no relationship between Diameter and Volume

$H_1: \beta_1 \neq 0$ , The artial regression slope is not equal to zero, i.e. There is a relationship between diameter and Volume


$H_0: \beta_1 = 0$ , The artial regression slope is equal to zero, i.e. there is no relationship between Height and Volume

$H_1: \beta_1 \neq 0$ , The artial regression slope is not equal to zero, i.e. There is a relationship between Height and Volume

Then we also test the overall model:

$H_0: \beta_1 = \beta_2 = 0$ (the all partial regression slopes are equal to zero, i.e. there is no relationship between diameter, height and volume)

$H_1: \beta_1 \neq 0$ and/or $\beta_2 \neq 0$ (at least one partial regression slope is not equal to zero, i.e.
there is a relationship between either between diameter and volume, or height and volume).
...
## Evaluate model fit of SLR and write up a conclusion.
...
The r^2^ value is,  which means that % of the variation in bird abundance can be explained by log10(Area). This is not really a strong r^2^ value, so it suggests that there are other factors influencing bird        abundance that are not included in the model.


## Evaluate model fit of MLR and write up a conclusion.
...
$\textrm{runoff_volume} = 3.257 + 0.05631\cdot \textrm{lake_sabrina} + 0.210\cdot \textrm{pine_creek} + 0.438\cdot \textrm{rock_creek} $
