...
We could use single values for each of our predictor variables and substitute each into the equation, but this is unrealistic and time consuming when we have a large number of observations. We therefore use a dataset containing our new observations of x to make the predictions and feed this into our model. 

To understand the true capability of our model to predict new values of y from values of x that have not been seen by the model, we need to test this on data that has not been used to fit the model. This is because if we test our model on the same data that we used to fit the model, we are likely to get an over-optimistic estimate of the model's predictive performance, as the model has already seen this data and may have overfitted to it.

To understand how well our model can predict new values of y from values of x which have not been used to fit the modeTo understand how well our model can predict new values of y from new values of x, there are a number of metrics and plots we can use, but 

Why are we separating the dataset?
