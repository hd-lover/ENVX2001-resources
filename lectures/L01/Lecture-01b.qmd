...
![Photo by <a href="https://unsplash.com/@brock222?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Richard Lee</a> on <a href="https://unsplash.com/photos/a-flock-of-birds-flying-over-a-dry-grass-field-XHxIXsIYEsQ?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>](images/snowgeese.jpg)
...
::: fragment
*Not all statistical techniques are inferential -- some are purely descriptive (e.g. summarising data with graphs or tables) -- but inference is the main focus in second year.*
:::
...
The study of how data are distributed -- pioneered by mathematicians such as Gauss -- underpins much of modern statistics. 

![Photo from Wikipedia.](images/gauss.jpg){fig-align="center"}
...
::::: columns
::: {.column width="40%"}
![Portrait of Friedrich Wilhelm Bessel (1784--1846)](images/bessel.jpg){fig-align="center"}
:::
::: {.column width="60%"}
...
sample_mean <- mean(sampling_dist$xbar)
theoretical_mean <- 2 # population mean for gamma(2,1)

ggplot(sampling_dist, aes(x = xbar)) +
  geom_density(
    fill = "orangered",
    alpha = 0.5, bins = 50
  ) +
  ...
  geom_vline(xintercept = theoretical_mean, color = "blue", linetype = "dashed") +
  geom_vline(xintercept = sample_mean, color = "red", linetype = "solid") +
  annotate("text",
    x = theoretical_mean, y = Inf,
    label = sprintf("Normal: %.2f", theoretical_mean),
    vjust = 2, hjust = -0.1, color = "blue"
  ) +
  annotate("text",
    x = sample_mean, y = Inf,
    label = sprintf("Sample: %.2f", sample_mean),
    vjust = 3.5, hjust = -0.1, color = "red"
  ) +
  xlab("Mean height (m)")
```
