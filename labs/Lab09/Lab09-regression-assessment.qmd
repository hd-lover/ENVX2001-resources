...
```{r}
mean(stream_train$runoff_volume - stream_train$pred) |> round(2)
mean(stream_test$runoff_volume - stream_test$pred) |> round(2)
```
...
```{r}
mean(abs(stream_train$runoff_volume - stream_train$pred)) |> round(2)
mean(abs(stream_test$runoff_volume - stream_test$pred)) |> round(2)
```
...
```{r}
library(caret)
RMSE(stream_train$runoff_volume, stream_train$pred) |> round(2)
RMSE(stream_test$runoff_volume, stream_test$pred) |> round(2)
```
...
```{r}
(cor(stream_train$runoff_volume, stream_train$pred))^2 |> round(2)
(cor(stream_test$runoff_volume, stream_test$pred))^2 |> round(2)
```

Option 2: `R2()` function from the `caret` package.

```{r}
R2(stream_train$runoff_volume, stream_train$pred) |> round(2)
R2(stream_test$runoff_volume, stream_test$pred) |> round(2)
```
...
```{r}
library(epiR)
epi.ccc(stream_train$runoff_volume, stream_train$pred)$rho.c$est |> round(2)
epi.ccc(stream_test$runoff_volume, stream_test$pred)$rho.c$est |> round(2)
```
...
##### *(iv)* Fit the linear model to the training subset using log(rel_fitness) as the response, and growth_rate and chlorogenic_acid as the predictor variables. 

```{r}
plant_train_fit <- lm(log(rel_fitness) ~ growth_rate + chlorogenic_acid, data = plant_train)
```
...
```{r}
#| code-fold: true
p1 <- ggplot(plant_train, aes(runoff_volume, pred)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(x = "Observed runoff volume (ML/year)", y = "Predicted runoff volume (ML/year)",
       title = "Training") +
...
p2 <- ggplot(plant_test, aes(runoff_volume, pred)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(x = "Observed runoff volume (ML/year)", y = "Predicted runoff volume (ML/year)",
       title = "Test") +
...
ggplot(plant_train, aes(runoff_volume, pred)) +
  geom_point() + 
  geom_smooth(data = train_ci, aes(fit, lwr), color = "blue", se = F) +
  geom_smooth(data = train_ci, aes(fit, upr), color = "blue", se = F) +
  geom_abline(intercept = 0, slope = 1, color = "maroon") + # 1:1 line
...
ggplot(plant_test, aes(runoff_volume, pred)) +
  geom_point() + 
  geom_smooth(data = test_pi, aes(fit, lwr), color = "red", se = F) +
  geom_smooth(data = test_pi, aes(fit, upr), color = "red", se = F) +
  geom_abline(intercept = 0, slope = 1, color = "maroon")  + # 1:1 line +
...
```{r}
mean(plant_train$runoff_volume - plant_train$pred) |> round(2)
mean(plant_test$runoff_volume - plant_test$pred) |> round(2)
```
...
```{r}
mean(abs(plant_train$runoff_volume - plant_train$pred)) |> round(2)
mean(abs(plant_test$runoff_volume - plant_test$pred)) |> round(2)
```
...
```{r}
library(caret)
RMSE(plant_train$runoff_volume, plant_train$pred) |> round(2)
RMSE(plant_test$runoff_volume, plant_test$pred) |> round(2)
```
...
```{r}
(cor(plant_train$runoff_volume, plant_train$pred))^2 |> round(2)
(cor(plant_test$runoff_volume, plant_test$pred))^2 |> round(2)
```

Option 2: `R2()` function from the `caret` package.

```{r}
R2(plant_train$runoff_volume, plant_train$pred) |> round(2)
R2(plant_test$runoff_volume, plant_test$pred) |> round(2)
```
...
```{r}
library(epiR)
epi.ccc(plant_train$runoff_volume, plant_train$pred)$rho.c$est |> round(2)
epi.ccc(plant_test$runoff_volume, plant_test$pred)$rho.c$est |> round(2)
```
...
training <- c(mean(plant_train$runoff_volume - plant_train$pred) |> round(3),
          mean(abs(plant_train$runoff_volume - plant_train$pred)) |> round(3),
          RMSE(plant_train$runoff_volume, plant_train$pred) |> round(3),
          R2(plant_train$runoff_volume, plant_train$pred) |> round(3),
          epi.ccc(plant_train$runoff_volume, plant_train$pred)$rho.c$est |> round(3))

test <- c(mean(plant_test$runoff_volume - plant_test$pred) |> round(3),
          mean(abs(plant_test$runoff_volume - plant_test$pred)) |> round(3),
          RMSE(plant_test$runoff_volume, plant_test$pred) |> round(3),
          R2(plant_test$runoff_volume, plant_test$pred) |> round(3),
          epi.ccc(plant_test$runoff_volume, plant_test$pred)$rho.c$est |> round(3))
