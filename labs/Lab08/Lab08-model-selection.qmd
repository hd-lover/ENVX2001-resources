...
# 1. Model quality - multiple vs adjusted *r*^2^ (10 mins)

You may have noticed how much we are focusing on the adjusted *r*^2^ value when interpreting our multiple linear regression models. This is because the multiple *r*^2^ value can be misleading when comparing models with different numbers of predictors.

In this exercise we will explore the difference between multiple *r*^2^ and adjusted *r*^2^ values and see how including a useless predictor can affect these values.

For this exercise we will use the same dataset as last week, which contains 43 years of annual precipitation measurements (in mm) taken at (originally) 6 sites in the Owens Valley in California. The response variable is the stream runoff volume measured at a site near Bishop, California (ML/year).
...
# 2. Variable selection for California_streamflow (~20 mins)
