...
For this lab, we will focus on the `check_model()` function from the `performance` package, which produces diagnostic plots that are easier to read than the base R `plot()` output. Compare the two approaches below to see why we recommend it:
...
![Kookaburra sitting on a branch, from Unsplash, by pen_ash.](images/pen_ash-mph8XzVCVkw-unsplash.jpg)
...
*(i)* Write out the statistical model for a simple linear regression and define it in terms of influence of forest patch area upon bird abundance. 
...
```{r}
#| fig-cap: "Figure 1. Boxplot of bird abundance"
...
```{r}
#| fig-cap: "Figure 2. Boxplot of area of forest patches (ha)"
...
```{r}
#| fig-cap: "Figure 3. Scatterplot of Bird abundance vs area of forest patch (ha)"
ggplot() +
...
```{r}
#| fig-cap: "Figure 3. Boxplot of Runoff volume (ML/year)"
...
```{r}
#| fig-cap: "Figure 4. Boxplot of precipitation at Lake Sabrina (mm)"
...
```{r}
#| fig-cap: "Figure 5. Boxplot of precipitation at Pine Creek (mm)"
...
```{r}
#| fig-cap: "Figure 6. Boxplot of precipitation at Rock Creek (mm)"
